...
export default {
  ...
  run: async (client, interaction) => {
    ...
    if (interaction.data.options[0].name === "view") {
      ...
      const user_id = interaction.data.options[0].options && interaction.data.options[0].options[0] ? interaction.data.options[0].options[0].value : interaction.member.id;
      ...
      let view = {
        ...
        fields: [
          ...
          {
            name: "Progress",
            value: `${userData.xp}/${userData.prestige * 100} to reach prestige ${userData.prestige + 1}\n\`[${xpBar}]\` `,
            inline: true
          }
        ],
        timestamp: new Date()
