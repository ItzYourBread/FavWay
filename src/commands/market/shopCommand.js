  ...
  run: async (client, interaction) => {
    if (interaction.data.options[0].name === 'view') {
      ...
      if (user && userData.settings.compactMode) {
        resource.map((el) => {
          resourceList += `${config.emojis[el.emoji]}**${el.name}** : ${config.emojis.coin}${Number(
            el.price,
          ).toLocaleString()}\n`;
        });
        item.map((el) => {
          itemList += `${config.emojis[el.emoji]}**${el.name}** : ${config.emojis.coin}${Number(
            el.price,
          ).toLocaleString()}\n`;
        });
        food.map((el) => {
          foodList += `${config.emojis[el.emoji]}**${el.name}** : ${config.emojis.coin}${Number(
            el.price,
          ).toLocaleString()}\n`;
        });
        crop.map((el) => {
          cropList += `${config.emojis[el.emoji]}**${el.name}** : ${config.emojis.coin}${Number(
            el.price,
          ).toLocaleString()}\n`;
        });
      }
      ...
      const quantity =
        interaction.data.options[0].options && interaction.data.options[0].options[1]
      ...
      let bought = {
        ...
        description: `Bought **${quantity} ${config.emojis[itemsList.emoji]}${
          itemsList.name
