...
export default {
    ...
    run: async (client, interaction) => {
        ...
        const userDataSpector =
            (await User.findOne({ id: interaction.member.id })) ||
            new User({ id: interaction.member.id });
        ...
        if (userDataSpector.settings.compactMode) {
            resource.map((el) => {
                if (
                    user &&
                    userData.resources[el.value] &&
                    userData.resources[el.value] >= 1
                ) {
                    Rescoures += `${config.emojis[el.emoji]}**${
                        el.name
                    }** : ${userData.resources[el.value].toLocaleString()}\n`;
                }
            });
            item.map((el) => {
                if (
                    user &&
                    userData.items[el.value] &&
                    userData.items[el.value] >= 1
                ) {
                    Items += `${config.emojis[el.emoji]}**${
                        el.name
                    }** : ${userData.items[el.value].toLocaleString()}\n`;
                }
            });
            food.map((el) => {
                if (
                    user &&
                    userData.foods[el.value] &&
                    userData.foods[el.value] >= 1
                ) {
                    Foods += `${config.emojis[el.emoji]}**${
                        el.name
                    }** : ${userData.foods[el.value].toLocaleString()}\n`;
                }
            });
            crop.map((el) => {
                if (
                    user &&
                    userData.crops[el.value] &&
                    userData.crops[el.value] >= 1
                ) {
                    Crops += `${config.emojis[el.emoji]}**${
                        el.name
                    }** : ${userData.crops[el.value].toLocaleString()}\n`;
                }
            });
        }
        if (!userDataSpector.settings.compactMode) {
        ...
        client.on('interactionCreate', async (i) => {
            if (
                ...
                i.data.custom_id === 'inventorySelectMenu'
            ) {
